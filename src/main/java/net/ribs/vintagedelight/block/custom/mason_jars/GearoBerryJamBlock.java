...
public class GearoBerryJamBlock extends MasonJarBlock {
    public GearoBerryJamBlock(Properties properties) {
        super(properties);
    }
    @Override
    public ItemStack getCloneItemStack(LevelReader world, BlockPos pos, BlockState state) {
        return new ItemStack(ModItems.GEARO_BERRY_JAM.get());
    }
    ...
    protected InteractionResult useWithoutItem(BlockState state, Level world, BlockPos pos, Player player, BlockHitResult hit) {
        if (!world.isClientSide) {
            ...
            if (jars > 1) {
                ...
                popResource(world, pos, new ItemStack(ModItems.GEARO_BERRY_JAM.get()));
